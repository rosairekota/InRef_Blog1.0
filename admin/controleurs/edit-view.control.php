                     ...
                     if (isset($_POST['submit']))
                     {
                       ...
                       // on commence a faire de verification
                         if (empty($title)|| empty($type) || empty($auteurs)||
                           ...
                           {
                                    modifier_Sans_file($title,$type, $content, $posted,$auteurs,$language);
                                    if (!empty($_FILES['image']['name']) && !empty($_FILES['fichier_pdf']['name']))
                                    {

                                        modifier_avec_file($tmp_pdf_name,$extension_pdf, $tmp_image_name,$extension_image,$path_final_all);

                                    }
                                    else
                                    {

                                            header("Location:index.php?page=edit-view&id=".$_GET['id']);
                                    }
                           }
